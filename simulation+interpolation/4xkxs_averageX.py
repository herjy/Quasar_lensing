...
while (R[-1] > epsilon) and (
    ...
    X_new1 = X + mu * ((A[0].T) @ (Y[0] - A[0] @ X))
    #X = X_new.copy()
    X_new2 = X + mu * ((A[1].T) @ (Y[1] - A[1] @ X))
    #X = X_new.copy()
    X_new3 = X + mu * ((A[2].T) @ (Y[2] - A[2] @ X))
    #X = X_new.copy()
    if count == 80:
        print(X_new3-X_new2)
    X_new4 = X + mu * ((A[3].T) @ (Y[3] - A[3] @ X))
    #X = X_new.copy()
    X = (np.array(X_new1)+np.array(X_new2)+np.array(X_new3)+np.array(X_new4))/4
