...
def simulate_noise(dt_max, noise=True):
    ...
    ts = np.loadtxt('t_drive_new.txt') # true time sample.

    ind = index(dt_max,ts) 
    # getting the indices of each time shifter curve
    index0, index1,index2,index3,index_truth = ind[0],ind[1],ind[2],ind[3],ind[4]
    sample = np.sort(ind[5])

    #getting the time sample of each curve
    ts_shifted = np.array([sample[index0],sample[index1],sample[index2],sample[index3]])
